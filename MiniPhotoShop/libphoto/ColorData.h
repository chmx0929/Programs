...
public:
	...
	/// Chroma red and Chroma blue are the other components of the YCbCr color space. These are used for Chroma key
	float getChromaBlueDifference() const;
	float getChromaRedDifference() const;

	/// Return a clamped version of this ColorData
	ColorData clampedColor() const;

	/// Round RGB value for ColorData, used in Dithering
	ColorData ColorDataFloor(ColorData input, float digit);
